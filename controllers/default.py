...
def index():
    ...
    revisora = (db.processo.user == auth.user_id) & (db.processo.competencia == "Revisora")
...
def pesquisa():
    ...
    if form.process().accepted:
        ...
        if form.vars.numero!="":
            numero = db.processo.numero.contains(form.vars.numero) & (db.processo.user == auth.user_id)
            query = db(numero)
            session.rows = query.select()
            redirect(URL('result'))
        
        elif form.vars.reu != "":
            reu = db.processo.reu.contains(form.vars.reu) & (db.processo.user == auth.user_id)
            query = db(reu)
            session.rows = query.select()
            redirect(URL('result'))
            ...
            competencia = (db.processo.competencia == form.vars.competencia) & (db.processo.user == auth.user_id)
            ...
            data = (db.processo.criado_em >= session.inicial-delta) & (db.processo.criado_em <= session.final+delta) & (db.processo.user == auth.user_id)
            query = db(data) 
            session.rows = query.select()
            redirect(URL('result'))   
